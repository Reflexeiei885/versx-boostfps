...
while($listener.IsListening){
 try{
  ...
  if($origin -and ($origin -match '^https?://localhost(:\d+)?$' -or $origin -match '^https?://127\.0\.0\.1(:\d+)?$' -or $origin -match '^https://.*\.vercel\.app$')){$res.Headers.Add('Access-Control-Allow-Origin',$origin)}
  $res.Headers.Add('Access-Control-Allow-Headers','Content-Type');$res.Headers.Add('Access-Control-Allow-Methods','GET,POST,OPTIONS')
